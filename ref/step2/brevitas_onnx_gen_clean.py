...
model_lstm = LSTMIDS()
path = "./model_29.pt" #Trained quantised model model 
i = 1
model_lstm.load_state_dict(torch.load(path),strict=False)#, map_location=device (Loading the model)
...
#     print(param)
print("------------------------------------------------------------------------")
#Note: This loop is used to figure out which quantizer has what scale value. These are later mapped to the ONNX implementation
...
# Exporting the model
model_lstm.eval() # Needs to be in the eval model for export
model_lstm(torch.randn(10, 1)) # Random Inputs
export_path = 'acm-transactions-cps-model.onnx' # Export file name
...
#------------------ Testing the model with some manual inputs ------------------------------------
