...
def lstm_weights_def(input_size=32, hidden_size=128, weights = None):
    initializer = {}
    print(f"weights[0].name: {weights[0].name}")
    print(f"weights[0].dims: {weights[0].dims}")
    print(f"weights[1].dims: {weights[1].dims}")
    print(f"input_size: {input_size}, hidden_size: {hidden_size}")
    # input gate
    bi_val = numpy_helper.to_array(weights[0]).reshape([1, input_size*2])
    Wi_val = numpy_helper.to_array(weights[1]).transpose()
    Ui_val = numpy_helper.to_array(weights[2]).transpose()
    # forget gate
    bf_val = numpy_helper.to_array(weights[3]).reshape([1, input_size*2])
    Wf_val = numpy_helper.to_array(weights[4]).transpose()
    Uf_val = numpy_helper.to_array(weights[5]).transpose()
    # cell gate
    bc_val = numpy_helper.to_array(weights[6]).reshape([1, input_size*2])
    Wc_val = numpy_helper.to_array(weights[7]).transpose()
    Uc_val = numpy_helper.to_array(weights[8]).transpose()
    # output gate
    bo_val = numpy_helper.to_array(weights[9]).reshape([1, input_size*2])
    Wo_val = numpy_helper.to_array(weights[10]).transpose()
    Uo_val = numpy_helper.to_array(weights[11]).transpose()
    # clip


    # set initializer
    # lstm weights
    initializer["W_f"] = make_tensor('W_f',onnx.TensorProto.FLOAT, [input_size, hidden_size], (Wf_val))
    initializer["U_f"] = make_tensor('U_f',onnx.TensorProto.FLOAT, [hidden_size,hidden_size], (Uf_val))
    initializer["b_f"] = make_tensor('b_f',onnx.TensorProto.FLOAT, [1, hidden_size], (bf_val))
    initializer["W_i"] = make_tensor('W_i',onnx.TensorProto.FLOAT, [input_size, hidden_size], (Wi_val))
    initializer["U_i"] = make_tensor('U_i',onnx.TensorProto.FLOAT, [hidden_size,hidden_size], (Ui_val))
    initializer["b_i"] = make_tensor('b_i',onnx.TensorProto.FLOAT, [1, hidden_size], (bi_val))
    initializer["W_c"] = make_tensor('W_c',onnx.TensorProto.FLOAT, [input_size, hidden_size], (Wc_val))
    initializer["U_c"] = make_tensor('U_c',onnx.TensorProto.FLOAT, [hidden_size,hidden_size], (Uc_val))
    initializer["b_c"] = make_tensor('b_c',onnx.TensorProto.FLOAT, [1, hidden_size], (bc_val))
    initializer["W_o"] = make_tensor('W_o',onnx.TensorProto.FLOAT, [input_size, hidden_size], (Wo_val))
    initializer["U_o"] = make_tensor('U_o',onnx.TensorProto.FLOAT, [hidden_size,hidden_size], (Uo_val))
    initializer["b_o"] = make_tensor('b_o',onnx.TensorProto.FLOAT, [1, hidden_size], (bo_val))
    # clip
    ...
    out_initializers = [initializer["W_f"], initializer["W_i"], initializer["W_c"], initializer["W_o"],
                        ...
                        initializer["min"], initializer["max"], initializer["min_6b"],
                        initializer["min_6b_2"], initializer["max_6b"],
...
def lstm_nodes_def(input_size=32, hidden_size=128):
    ...
    nodes["inp2_m2"] = make_tensor_value_info("h_t-1",onnx.TensorProto.FLOAT, [1, hidden_size])
    
    nodes["inp2_elm1"] = make_tensor_value_info("c_t-1", onnx.TensorProto.FLOAT, [1, hidden_size])
    nodes["inp2_m1"] = make_tensor_value_info("X",onnx.TensorProto.FLOAT, [1, input_size])


    nodes["out_cell_state"] = make_tensor_value_info("c_t_out", onnx.TensorProto.FLOAT, [1, hidden_size])
    nodes["out_hidden_state_float"] = make_tensor_value_info("dql_hidden_out", onnx.TensorProto.FLOAT, [1, hidden_size])  
    ...
    nodes["mul_node1_e1"] = make_node("MatMul", inputs=["dql_input_out","dql_wf_out"], outputs=["out_m1_e1"], name="mul_node1_e1") #As io_quant was none during training. Hence setting the input as original input. We know it's UINT8 so we set that in FINN.
    nodes["mul_node2_e1"] = make_node("MatMul", inputs=["input_dql_hidden_out","dql_uf_out"], outputs=["out_m2_e1"],name="mul_node2_e1")
    ...
    nodes["mul_node1_e2"] = make_node("MatMul", inputs=["dql_input_out","dql_wi_out"], outputs=["out_m1_e2"], name="mul_node1_e2")
    nodes["mul_node2_e2"] = make_node("MatMul", inputs=["input_dql_hidden_out","dql_ui_out"], outputs=["out_m2_e2"],name="mul_node2_e2")
    ...
    nodes["mul_node1_e3"] = make_node("MatMul", inputs=["dql_input_out", "dql_wo_out"], outputs=["out_m1_e3"], name="mul_node1_e3")
    nodes["mul_node2_e3"] = make_node("MatMul", inputs=["input_dql_hidden_out", "dql_uo_out"], outputs=["out_m2_e3"],name="mul_node2_e3")
    ...
    nodes["mul_node1_e4"] = make_node("MatMul", inputs=["dql_input_out", "dql_wc_out"], outputs=["out_m1_e4"], name="mul_node1_e4")
    nodes["mul_node2_e4"] = make_node("MatMul", inputs=["input_dql_hidden_out","dql_uc_out"], outputs=["out_m2_e4"],name="mul_node2_e4")
    ...
    nodes["el_mul_node1_e5"] = make_node("Mul", inputs=["dql_cell_out","f_t_dql2"], outputs=["out_el_mul1_e5"],name="el_mul_node1_e5") #c_t-1
    ...
    nodes["el_mul_node2_e5"] = make_node("Mul", inputs=["c_t_dql2","i_t_dql2"], outputs=["out_el_mul2_e5"], name="el_mul_node2_e5")
    ...
    nodes["el_mul_node1_e6"] = make_node("Mul", inputs=["o_t_dql2","sixth_dql1"], outputs=["h_t_inter"], name="el_mul_node1_e6")#h_t_inter : Curent Hidden State output
    #Branch that gives the output
    ...
    nodes["dql_hidden_out_carry"] = make_tensor_value_info("dql_hidden_out_carry", onnx.TensorProto.FLOAT, [1, hidden_size])

    return nodes

def lstm_values_def(input_size=32, hidden_size=128):
    values = [
            make_tensor_value_info("ql_input_out",onnx.TensorProto.INT8, [1, input_size]),
            # make_tensor_value_info("ql_input_out_clip",onnx.TensorProto.UINT8, [input_size,1]),	
            make_tensor_value_info("dql_input_out",onnx.TensorProto.FLOAT, [1, input_size]),
            make_tensor_value_info("input_ql_hidden_out",onnx.TensorProto.INT8, [1, hidden_size]),
            # make_tensor_value_info("input_ql_hidden_out_clip",onnx.TensorProto.UINT8, [hidden_size,1]),
            make_tensor_value_info("input_dql_hidden_out",onnx.TensorProto.FLOAT, [1, hidden_size]),
            make_tensor_value_info("ql_hidden_out",onnx.TensorProto.INT8, [1, hidden_size]),
            make_tensor_value_info("ql_hidden_out_carry",onnx.TensorProto.INT8, [1, hidden_size]),
            ...
            make_tensor_value_info("h_t_inter_identity",onnx.TensorProto.FLOAT, [1, hidden_size]),
            make_tensor_value_info("ql_cell_out",onnx.TensorProto.INT8, [1, hidden_size]),
            make_tensor_value_info("dql_cell_out",onnx.TensorProto.FLOAT, [1, hidden_size]),
            make_tensor_value_info("out_m1_e1",onnx.TensorProto.FLOAT, [1, hidden_size]),
            make_tensor_value_info("out_m2_e1",onnx.TensorProto.FLOAT, [1, hidden_size]),
            make_tensor_value_info("out_add1_e1",onnx.TensorProto.FLOAT, [1, hidden_size]),
            make_tensor_value_info("f_t_ba",onnx.TensorProto.FLOAT, [1, hidden_size]),
            make_tensor_value_info("f_t_ql1",onnx.TensorProto.INT8, [1, hidden_size]),#Output of the quantized linear layer. Therefore the datatype INT8.
            make_tensor_value_info("clp_f_t_ql1",onnx.TensorProto.INT8, [1, hidden_size]),#Output of the quantized linear layer. Therefore the datatype INT8.
            make_tensor_value_info("f_t_dql1", onnx.TensorProto.FLOAT, [1, hidden_size]),#Output of the dequantized linear layer. Therefore float datatype as the input will now be processed differently.
            make_tensor_value_info("f_t_ql2",onnx.TensorProto.UINT8, [1, hidden_size]),#Output of the quantized linear layer. Therefore the datatype INT8.
            make_tensor_value_info("clp_f_t_ql2",onnx.TensorProto.UINT8, [1, hidden_size]),#Output of the quantized linear layer. Therefore the datatype INT8.
            make_tensor_value_info("f_t_dql2", onnx.TensorProto.FLOAT, [1, hidden_size]),#Output of the dequantized linear layer. Therefore float datatype as the input will now be processed differently.
            make_tensor_value_info("out_m1_e2",onnx.TensorProto.FLOAT, [1, hidden_size]),
            make_tensor_value_info("out_m2_e2",onnx.TensorProto.FLOAT, [1, hidden_size]),
            make_tensor_value_info("out_add1_e2",onnx.TensorProto.FLOAT, [1, hidden_size]),
            make_tensor_value_info("i_t_ba",onnx.TensorProto.FLOAT, [1, hidden_size]),
            make_tensor_value_info("i_t_ql1",onnx.TensorProto.INT8, [1, hidden_size]),#Output of the quantized linear layer. Therefore the datatype INT8.
            make_tensor_value_info("clp_i_t_ql1",onnx.TensorProto.INT8, [1, hidden_size]),#Output of the quantized linear layer. Therefore the datatype INT8.
            make_tensor_value_info("i_t_dql1", onnx.TensorProto.FLOAT, [1, hidden_size]),#Output of the dequantized linear layer. Therefore float datatype as the input will now be processed differently.
            make_tensor_value_info("i_t_ql2",onnx.TensorProto.UINT8, [1, hidden_size]),#Output of the quantized linear layer. Therefore the datatype INT8.
            make_tensor_value_info("clp_i_t_ql2",onnx.TensorProto.UINT8, [1, hidden_size]),#Output of the quantized linear layer. Therefore the datatype INT8.
            make_tensor_value_info("i_t_dql2", onnx.TensorProto.FLOAT, [1, hidden_size]),#Output of the dequantized linear layer. Therefore float datatype as the input will now be processed differently.
            make_tensor_value_info("out_m1_e3",onnx.TensorProto.FLOAT, [1, hidden_size]),
            make_tensor_value_info("out_m2_e3",onnx.TensorProto.FLOAT, [1, hidden_size]),
            make_tensor_value_info("out_add1_e3",onnx.TensorProto.FLOAT, [1, hidden_size]),
            make_tensor_value_info("o_t_ba",onnx.TensorProto.FLOAT, [1, hidden_size]),
            make_tensor_value_info("o_t_ql1",onnx.TensorProto.INT8, [1, hidden_size]),#Output of the quantized linear layer. Therefore the datatype INT8.
            make_tensor_value_info("clp_o_t_ql1",onnx.TensorProto.INT8, [1, hidden_size]),#Output of the quantized linear layer. Therefore the datatype INT8.
            make_tensor_value_info("o_t_dql1", onnx.TensorProto.FLOAT, [1, hidden_size]),#Output of the dequantized linear layer. Therefore float datatype as the input will now be processed differently.
            make_tensor_value_info("o_t_ql2",onnx.TensorProto.UINT8, [1, hidden_size]),#Output of the quantized linear layer. Therefore the datatype INT8.
            make_tensor_value_info("clp_o_t_ql2",onnx.TensorProto.UINT8, [1, hidden_size]),#Output of the quantized linear layer. Therefore the datatype INT8.
            make_tensor_value_info("o_t_dql2", onnx.TensorProto.FLOAT, [1, hidden_size]),#Output of the dequantized linear layer. Therefore float datatype as the input will now be processed differently.
            make_tensor_value_info("out_m1_e4",onnx.TensorProto.FLOAT, [1, hidden_size]),
            make_tensor_value_info("out_m2_e4",onnx.TensorProto.FLOAT, [1, hidden_size]),
            make_tensor_value_info("out_add1_e4",onnx.TensorProto.FLOAT, [1, hidden_size]),
            make_tensor_value_info("c_t_ba",onnx.TensorProto.FLOAT, [1, hidden_size]),
            make_tensor_value_info("c_t_ql1",onnx.TensorProto.INT8, [1, hidden_size]),#Output of the quantized linear layer. Therefore the datatype INT8.
            make_tensor_value_info("clp_c_t_ql1",onnx.TensorProto.INT8, [1, hidden_size]),#Output of the quantized linear layer. Therefore the datatype INT8.
            make_tensor_value_info("c_t_dql1", onnx.TensorProto.FLOAT, [1, hidden_size]),#Output of the dequantized linear layer. Therefore float datatype as the input will now be processed differently.
            make_tensor_value_info("c_t_ql2",onnx.TensorProto.INT8, [1, hidden_size]),#Output of the quantized linear layer. Therefore the datatype INT8.
            make_tensor_value_info("clp_c_t_ql2",onnx.TensorProto.INT8, [1, hidden_size]),#Output of the quantized linear layer. Therefore the datatype INT8.
            make_tensor_value_info("c_t_dql2", onnx.TensorProto.FLOAT, [1, hidden_size]),#Output of the dequantized linear layer. Therefore float datatype as the input will now be processed differently.
            make_tensor_value_info("f_t",onnx.TensorProto.FLOAT, [1, hidden_size]),
            make_tensor_value_info("i_t",onnx.TensorProto.FLOAT, [1, hidden_size]),
            make_tensor_value_info("o_t",onnx.TensorProto.FLOAT, [1, hidden_size]),
            make_tensor_value_info("c_t_partial",onnx.TensorProto.FLOAT, [1, hidden_size]),
            make_tensor_value_info("out_el_mul1_e5",onnx.TensorProto.FLOAT, [1, hidden_size]),
            make_tensor_value_info("out_el_mul2_e5",onnx.TensorProto.FLOAT, [1, hidden_size]),
            make_tensor_value_info("fifth_ql1",onnx.TensorProto.INT8, [1, hidden_size]),#Output of the quantized linear layer. Therefore the datatype INT8.
            make_tensor_value_info("clp_fifth_ql1",onnx.TensorProto.INT8, [1, hidden_size]),#Output of the quantized linear layer. Therefore the datatype INT8.
            make_tensor_value_info("fifth_dql1", onnx.TensorProto.FLOAT, [1, hidden_size]),#Output of the dequantized linear layer. Therefore float datatype as the input will now be processed differently.
            make_tensor_value_info("fifth_ql2",onnx.TensorProto.INT8, [1, hidden_size]),#Output of the quantized linear layer. Therefore the datatype INT8.
            make_tensor_value_info("clp_fifth_ql2",onnx.TensorProto.INT8, [1, hidden_size]),#Output of the quantized linear layer. Therefore the datatype INT8.
            make_tensor_value_info("fifth_dql2", onnx.TensorProto.FLOAT, [1, hidden_size]),#Output of the dequantized linear layer. Therefore float datatype as the input will now be processed differently.
            make_tensor_value_info("h_t_ql",onnx.TensorProto.INT8, [1, hidden_size]),#Output of the quantized linear layer. Therefore the datatype INT8.
            make_tensor_value_info("clp_h_t_ql",onnx.TensorProto.INT8, [1, hidden_size]),#Output of the quantized linear layer. Therefore the datatype INT8.
            make_tensor_value_info("c_t_carry_ql",onnx.TensorProto.INT8, [1, hidden_size]),#Output of the quantized linear layer. Therefore the datatype INT8.
            make_tensor_value_info("clp_c_t_carry_ql",onnx.TensorProto.INT8, [1, hidden_size]),#Output of the quantized linear layer. Therefore the datatype INT8.
            make_tensor_value_info("c_t_carry_dql", onnx.TensorProto.FLOAT, [1, hidden_size]),
            ...
            make_tensor_value_info("c_t", onnx.TensorProto.FLOAT, [1, hidden_size]),#Output of the dequantized linear layer. Therefore float datatype as the input will now be processed differently.
            make_tensor_value_info("out_tanh_e6",onnx.TensorProto.FLOAT, [1, hidden_size]),
            make_tensor_value_info("sixth_ql1",onnx.TensorProto.INT8, [1, hidden_size]),#Output of the quantized linear layer. Therefore the datatype INT8.
            make_tensor_value_info("clp_sixth_ql1",onnx.TensorProto.INT8, [1, hidden_size]),#Output of the quantized linear layer. Therefore the datatype INT8.
            make_tensor_value_info("sixth_dql1", onnx.TensorProto.FLOAT, [1, hidden_size]),#Output of the dequantized linear layer. Therefore float datatype as the input will now be processed differently.
            make_tensor_value_info("sixth_ql2",onnx.TensorProto.INT8, [1, hidden_size]),#Output of the quantized linear layer. Therefore the datatype INT8.
            make_tensor_value_info("h_t_inter", onnx.TensorProto.FLOAT, [1, hidden_size]),#Output of the dequantized linear layer. Therefore float datatype as the input will now be processed differently.
            make_tensor_value_info("ql_wf_out", onnx.TensorProto.INT8, [input_size, hidden_size]),
            make_tensor_value_info("dql_wf_out",onnx.TensorProto.FLOAT, [input_size, hidden_size]),
            make_tensor_value_info("ql_wi_out", onnx.TensorProto.INT8, [input_size, hidden_size]),
            make_tensor_value_info("dql_wi_out",onnx.TensorProto.FLOAT, [input_size, hidden_size]),
            make_tensor_value_info("ql_wc_out", onnx.TensorProto.INT8, [input_size, hidden_size]),
            make_tensor_value_info("dql_wc_out",onnx.TensorProto.FLOAT, [input_size, hidden_size]),
            make_tensor_value_info("ql_wo_out", onnx.TensorProto.INT8, [input_size, hidden_size]),
            make_tensor_value_info("dql_wo_out",onnx.TensorProto.FLOAT, [input_size, hidden_size]),
            ...
            make_tensor_value_info("clp_wf",onnx.TensorProto.INT8, [input_size, hidden_size]),
            make_tensor_value_info("clp_wi",onnx.TensorProto.INT8, [input_size, hidden_size]),
            make_tensor_value_info("clp_wc",onnx.TensorProto.INT8, [input_size, hidden_size]),
            make_tensor_value_info("clp_wo",onnx.TensorProto.INT8, [input_size, hidden_size]),
            ...
            make_tensor_value_info("clp_cell_6b",onnx.TensorProto.INT8, [1, hidden_size]),
