...
def data_init(batch_size=32, input_cycle_fraction=1, delete_temp_files=False):
    ...
    voltage_input_scale = voltage_loaders['train_input_scale']
    voltage_output_scale = voltage_loaders['train_output_scale']
    current_input_scale = current_loaders['train_input_scale']
    current_output_scale = current_loaders['train_output_scale']
    simulation_input_scale = simulation_loaders['train_input_scale']
    simulation_output_scale = simulation_loaders['train_output_scale']

    # Denormalize and visualize samples
    sample_input, sample_output, vehicle_id = current_loaders['train'].dataset[0]
    sample_input_denorm = denormalize(sample_input, current_input_scale)
    sample_output_denorm = denormalize(sample_output, current_output_scale)
    processor.visualize_sample(sample_input_denorm, sample_output_denorm, vehicle_id, 
    ...
    sample_input_denorm = denormalize(sample_input, voltage_input_scale)
    sample_output_denorm = denormalize(sample_output, voltage_output_scale)
    processor.visualize_sample(sample_input_denorm, sample_output_denorm, vehicle_id, 
    ...
    sample_input_denorm = denormalize(sample_input, simulation_input_scale)
    sample_output_denorm = denormalize(sample_output, simulation_output_scale)
    processor.visualize_sample(sample_input_denorm, sample_output_denorm, vehicle_id, 
                              signal_type='current', name=f'current_simu_{input_cycle_fraction}', show_fig=False)

    # Print batch info (optional: denormalize batch data if needed for printing)
